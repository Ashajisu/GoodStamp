...
function createDailyTodosForToday() {
    ...
    if (state.todos.some(todo => todo.date === today && todo.repeat === 'daily')) {
        return
    }
    ...
    const latestDate = dailyTodos.reduce(
        (latest, todo) => todo.date > latest ? todo.date : latest,
        ''
    )
...
function updateProgress(todo, progress) {
    ...
    if (todo.progress >= todo.requiredProgress && !todo.completed) {
        toggleTodo(todo)
    }
...
function exchangeReward(reward) {
    if (state.stamps.length < reward.required) {
        return false
    }
...
export {state,todayTodos,todayKey,stamps,login,logout,addTodo,deleteTodo,toggleTodo,updateProgress,addReward,updateReward, deleteReward,exchangeReward,save, weekStamps, usedStampCount, totalStampCount, getWeekMonday}
